/*
 This my personal boilerplate code, I just started this.
 */

/*
 es6-like promise implementation
...
/**
 http://stackoverflow.com/a/22184713
 ...
 **/
jcumines.csvWriter = function (del, enc) {
...
/**
 Returns a promise containing the file contents, or an error.
 */
jcumines.loadFile = function (domString) {
    try {
        return new Promise(function (fulfill, reject) {
            var rawFile = new XMLHttpRequest();
            rawFile.open("GET", domString, true);
            rawFile.onreadystatechange = function () {
                if (rawFile.readyState === 4) {
                    if (rawFile.status === 200 || rawFile.status == 0) {
                        fulfill(rawFile.responseText);
                    } else {
                        reject(new Error('Unable to load the file.'));
                    }
                }
            };
            rawFile.send(null);
...
jcumines.clearInputFile = function (f) {
    if (f.value) {
        try {
